...
class TestExtractionsUsingRegex(unittest.TestCase):
    ...
    def test_extractions_structured_tokens(self):
        ...
        # Check filtering
        filtered_tokens = self.c.filter_tokens(
            extracted_tokens, ['alphabet', 'punctuation'])

        self.assertEquals(filtered_tokens.tokens, self.correct_filtered_tokens)

        # Check reverse map
        self.assertEquals(filtered_tokens.reverse_map,
                          self.correct_reverse_map)

        # Check getting reverse index
        self.assertEquals(filtered_tokens.get_original_index(3), 6)
